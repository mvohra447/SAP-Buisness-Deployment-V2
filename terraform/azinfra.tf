...
resource "azurerm_resource_group" "rg" {
  name     = "${var.linux_vm_name}_${var.resource_group}"
...
resource "azurerm_virtual_network" "vnet" {
  name                = "${var.linux_vm_name}_${var.virtual_network_name}"
...
resource "azurerm_subnet" "subnet1" {
  name                 = "${var.linux_vm_name}_${var.azure_subnet1_name}"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
...
resource "azurerm_subnet" "subnet2" {
  name                 = "${var.linux_vm_name}_${var.azure_subnet2_name}"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
  virtual_network_name = "${azurerm_virtual_network.vnet.name}"
...
resource "azurerm_network_security_group" "nsg" {
  name                = "${var.linux_vm_name}_${var.nsg_name}"
  location            = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  security_rule {
...
resource "azurerm_network_interface" "linuxnic" {
  name                = "${var.linux_vm_name}_${var.linuxnic}"
  location            = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
...
resource "azurerm_network_interface" "winnic" {
  name                = "${var.linux_vm_name}_${var.winnic}"
  location            = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  network_security_group_id = "${azurerm_network_security_group.nsg.id}"
...
resource "azurerm_virtual_machine" "winvm" {
  ...
  count                 = "${var.win_rdp_create == "yes" ? 1 : 0}"
...
resource "azurerm_managed_disk" "disk1" {
  name                 = "${var.linux_vm_name}_UsrSap_Disk1"
  location             = "eastus"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
...
resource "azurerm_managed_disk" "disk2" {
  name                 = "${var.linux_vm_name}_H1BData_Disk1"
  location             = "eastus"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_managed_disk" "disk3" {
  name                 = "${var.linux_vm_name}_H1BLog_Disk1"
  location             = "eastus"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_managed_disk" "disk4" {
  name                 = "${var.linux_vm_name}_H1BShared_Disk1"
  location             = "eastus"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
